...
/** \author Jorgen Bodde
    ...
    This is the wxArchive documentation guide. wxArchive is a serializing class meant for serializing / de-serializing
	a complete (or partial) object model to a stream. The streaming is made type safe and reliable. This means when an
	error occurs (wrong data read back or unexpected end of file) the wxArchive class logs an error, and refuses to
	...
	wrote a stream with version 1000, and upon reading you expect version 900, the wxArchive class logs an error that
	...
	Because wxArchive uses wxInputStream and wxOutputStream for reading and writing, the contents of the serialization
	...
	\code
	bool MyDataClass::Serialize(wxArchive &a)
	{
		if(a.IsStoring())
		{
    ...
    -	<b>Reading a newer stream with an older application</b>. When the application you released in the field is older then the stream
		...
		in the stream, so wxArchive will go look for the boundary and ignore all data that is newer.
	...
	caution (for example using the version tag or wxArchiveStatus object) in your application to warn the user that
	...
	\subsection archive_status wxArchiveStatus Helper Class
	...
	using the wxArchiveStatus object earlier retrieved when reading will prevent the write if the flag NewDataLoss()
	...
	This wxArchiveStatus class will also remove the need of writing the same header back as being specified when
	reading, and also the version will be preserved. The member wxArchiveStatus::SetVersion() can be used to specify
	...
	{
		wxFileOutputStream file("somefile.dat");
		wxArchive a(file, m_ourStatus.GetVersion(), m_ourStatus.GetHeader());
		// serialize data
		m_ourClass->Serialize(a);
	}
	else
...
{
public:
	/** Constructor of the wxArchiveStatus object. This is the default constructor so you can
	...
	/** Constructor initializer of the wxArchiveStatus object. Used by wxArchive to fill in the goods
	    so they can be used later */
	wxArchiveStatus(size_t version, const wxString &header)
		: m_version(version)
		, m_header(header)
		, m_newDataLoss(false)
	{
	...
	/** Copy operator to get the latest copy of it from the wxArchive class, and assign it to your copy */
	void operator=(const wxArchiveStatus &s) {
		m_version = s.m_version;
		m_header = s.m_header;
		m_newDataLoss = s.m_newDataLoss;
	};

	/** With this method you tell wxArchive that it is ok to overwrite the newer data stream with an older
	...
	*/
	...
	/** This method returns the flag of the current NewDataLoss status. It will be set when the wxArchive
	...
	*/
	...
	/** Sets the NewDataLoss flag again. This is used by wxArchive only, or you can use it to prevent other
...
public:
    /** Construct an input stream archive. The stream will be used to retrieve the data from, which can be any
		derived stream like a file, zip stream etc. When constructed as input, the wxArchive object cannot
		be written to.
        ...
        When setting the version parameter, wxArchive will check if the version read back is at least the expected
		version. For example, when version argument is set to 1000, wxArchive will accept versions 0 .. 1000 for reading
		...
		wxArchive a(in_stream, 1000, _T("My Stream"));
        if(a.IsOk())
        {
            a >> m_name;
            a >> m_age;
		...
		\sa IsFullSerialize(), wxArchiveStatus

    */
    wxArchive(wxInputStream &stream, size_t version = 0, const wxString &header = wxEmptyString, bool partialMode = false);

    /** Construct an output stream object. The stream will be used to save the data to, which can be any derived
	    stream like a file, zip stream etc. When constructed as output, the wxArchive object cannot be read from.

        \code
        // open and write
        wxFileOutputStream out_file("somefile.dat");
		wxArchive a(out_file, 1000, _T("My Stream"));
        a << m_name;
        a << m_age;
        a << m_string << m_count;
    ...
    */
    wxArchive(wxOutputStream &stream, size_t version = 1, const wxString &header = wxEmptyString, bool partialMode = false);
    ...
    /** In reading mode, it returns true when the stream we are reading from returns true on Eof().
        ...
        In writing mode Eof() returns always false.
        When the stream is closed, true is always returned, we can't read or write anymore in that
		case anyway. Reading more from a stream that is at it's end will produce errors.
	*/
    bool Eof();

	/** Returns true when the wxArchive is in OK condition. This means no error occured. Errors that can occur are
	    for example:
	        ...
	        - Reading from a wxArchive with a higher version then supported
	        - Reading from a wxArchive with a wrong header
	        - Writing to the wxArchive when we are in reading mode
	        - Reading from the wxArchive when we are in writing mode

	    See GetLastError() to retrieve the error, and GetLastErrorString() for an explanation. When the wxArchive
	    ...
	    \code
	    wxArchive a(in_stream);

	    // .. some code

        if(a.IsStoring())
        {
            // when wxArchive is not ok, this does nothing
	...
	*/
	bool IsOk() {
		bool noerr = (m_errorCode == wxARCHIVE_ERR_OK);
		if(m_writeMode)
	...
	/** This method allows you to specify regions of an object that wxArchive can identify. This allows
		...
		the wxArchive class stays in an error state so it won't be harmful trying to read or write anyway.
	...
	*/
	...
	/** Indicates current state is writing. This flag will always be true if wxArchive is constructed with a
    ...
    */
    ...
    /** Indicates current state is reading. This flag will always be true if wxArchive is constructed with a
    ...
    */
    ...
    /** This is an extra flag independent of IsStoring() or IsLoading(). It can be set to indicate that certain
        ...
        \code
        bool SomeObject::Serialize(wxArchive &a)
        {
            if(a.IsStoring())
            {
                // store our data
    ...
    */
    ...
    virtual wxArchive &operator <<(wxUint8 value) {
        WriteUint8(value);
	    return *this;
    };

    /** Convenience operator for serializing. See WriteUInt16() */
    virtual wxArchive &operator <<(wxUint16 value) {
        WriteUint16(value);
	    return *this;
    };

    /** Convenience operator for serializing. See WriteUInt32() */
    virtual wxArchive &operator <<(wxUint32 value) {
        WriteUint32(value);
	    return *this;
    };

    /** Convenience operator for serializing. See WriteUInt64() */
	virtual wxArchive &operator <<(wxUint64 value) {
        WriteUint64(value);
	    return *this;
    };

    /** Convenience operator for serializing. See WriteInt() */
    virtual wxArchive &operator <<(int value) {
        WriteInt(value);
	    return *this;
    };

	/** Convenience operator for serializing. See WriteString() */
	virtual wxArchive &operator <<(const wxString& value) {
        WriteString(value);
	    return *this;
    };

	/** Convenience operator for serializing. See WriteArrayString() */
	virtual wxArchive &operator <<(const wxArrayString& value) {
        WriteArrayString(value);
	    return *this;
    };

    /** Convenience operator for serializing. See WriteString() */
	virtual wxArchive &operator <<(const wxChar* pvalue) {
	...
	virtual wxArchive &operator <<(bool value) {
        WriteBool(value);
	    return *this;
    };

    /** Convenience operator for serializing. See WriteDouble() *
    virtual wxArchive &operator <<(wxFloat64 value) {
        WriteDouble(value);
	    return *this;
    };*/
	...
	virtual wxArchive &operator >>(wxString& value) {
        ReadString(value);
		return *this;
    };

	/** Convenience operator for serializing. See ReadArrayString() */
	virtual wxArchive &operator >>(wxArrayString& value) {
	...
	/** Loads a boolean from stream. When the next record to be read is not a bool, an error will occur.
	    When wxArchive encountered the end of object boundary (see \ref enter_leave) or the archive is
    ...
    */
	...
	/** Loads a double from stream. When the next record to be read is not a double, an error will occur.
	    When wxArchive encountered the end of object boundary (see \ref enter_leave) or the archive is
		in an error state, the value is not touched.
		...
		The value returned is true when the read was succesful, false when nothing was read. The double is
		stored as a IEEE converted standard double, and is x-platform so it can be shared between other os'es
    */
    ...
    */
	...
	/** Loads a wxArrayString from stream. When the next record to be read is not a wxArrayString, an error
		will occur.  When wxArchive encountered the end of object boundary (see \ref enter_leave) or the archive is
		in an error state, the value is not touched.
    ...
    */
    ...
    /** Saves a double to stream. It writes a header type first, and after that the value. This means when the double
	    is read back when it is not expected, an error occurs and the program can terminate gracefully because no
		bogus values are read back. True is returned when the value is saved properly, and false when it's not.

        The double is saved in a cross platform compatible mode, which means the streams can be interchanged between
    ...
    */
	...
	/** Returns the wxArchiveStatus object. You usually do this when you just read a stream, to get more
	...
	const wxArchiveStatus &GetArchiveStatus() const {
		return m_status;
	};
...
private:
	...
	enum
	{
		wxARCHIVE_ERR_STR_HEADER_s1_s2,         /// Illegal header in start of stream
		wxARCHIVE_ERR_STR_WRONGVERSION_s1_s2,   /// Not a proper version
		wxARCHIVE_ERR_STR_BADISTREAM,           /// Bad input stream
		wxARCHIVE_ERR_STR_BADOSTREAM,           /// Bad output stream
		wxARCHIVE_ERR_STR_NOVERHDR,             /// Cannot write version or header info to stream
		wxARCHIVE_ERR_STR_NOHEADER_s1,          /// Header expected but not found
		wxARCHIVE_ERR_STR_NOVERSION,            /// No version found in stream
		wxARCHIVE_ERR_STR_NOWRITE,              /// Cannot write in read mode
		wxARCHIVE_ERR_STR_NOREAD,               /// Cannot read in write mode
        wxARCHIVE_ERR_STR_EOF, 	                /// End of file error in read mode
        wxARCHIVE_ERR_STR_WRONGCHUNK_s1_s2,     /// Expected header x but got y
        wxARCHIVE_ERR_STR_MEMORY_s1,            /// Cannot allocate x bytes of memory
		wxARCHIVE_ERR_STR_READSIZE,				/// Read size mismatch
		wxARCHIVE_ERR_STR_RINTSIZE,				/// Integer size mismatch
		wxARCHIVE_ERR_STR_SINTSIZE,				/// Ditto
		wxARCHIVE_ERR_STR_ILL_LEAVE,			/// Illegal leave marker found
		wxARCHIVE_ERR_STR_ILL_UNKNOWN_HDR_s1,	/// Unknown header error
		wxARCHIVE_ERR_STR_ILL_LEVEL				/// Sync level failure!
	};

	// headers
	enum
	{
		wxARCHIVE_HDR_BOOL      = 'b',
		wxARCHIVE_HDR_INT8      = 'c',
		wxARCHIVE_HDR_INT16     = 'w',
		wxARCHIVE_HDR_INT32     = 'l',
		wxARCHIVE_HDR_INT64     = 'q',
		wxARCHIVE_HDR_DOUBLE    = 'd',
		wxARCHIVE_HDR_STRING    = 's',
		wxARCHIVE_HDR_RECORD    = 'r',
		wxARCHIVE_HDR_INT       = 'i',
		wxARCHIVE_HDR_ARRSTRING = 'a',
		wxARCHIVE_HDR_ENTER     = '<',
		wxARCHIVE_HDR_LEAVE     = '>'
	};
	...
	*/
	void SetError(int err) {
		...
		if(m_errorCode == wxARCHIVE_ERR_OK)
			m_errorCode = err;
	};
...
private:
	// last error code issued in wxArchive
	int m_errorCode;
