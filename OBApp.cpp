...
OBMainFrame* OBApp::spMainFrame_ = NULL;

/*static*/ OBMainFrame* OBApp::MainFrame ()
{
    return spMainFrame_;
}
...
bool OBApp::OnInit()
{
    ...
    // init main frame
    spMainFrame_ = new OBMainFrame(*this);
...
bool OBApp::OpenProject (const wxChar* filename)
{
    return OBRootTask::Instance().ReadFromFile(filename);
}

bool OBApp::SaveProject (const wxChar* filename)
{
    return OBRootTask::Instance().StoreToFile(filename);
}

bool OBApp::IsProjectModified ()
{
    return OBRootTask::Instance().IsModified();
}
...
void OBApp::Backup()
{
    ...
    // init
    OBRootTask& rRoot = OBRootTask::Instance();
